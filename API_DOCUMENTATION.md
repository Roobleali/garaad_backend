...
#### Validate Diagrammar State
```http
POST /problems/{problem_id}/validate-diagrammar/
```
**Purpose**: Validates a user's diagram state against the correct solution for a Diagrammar problem. This endpoint provides immediate feedback on whether the diagram is correct and offers hints if needed.
...
```json
{
    "is_correct": boolean,
    "message": "string",
    "hint": "string"  // Optional, only provided if the diagram is incorrect
}
```
...
```bash
curl -X POST \
  'https://api.garaad.org/api/lms/problems/1/validate-diagrammar/' \
...
```json
{
    ...
    "hint": "Check the positions of your elements."
}
```
